...
@onready var door = $"../NavigationRegion3D/stairs/stair3/Door/hinge/StaticBody3D"
var open
...
func _on_area_3d_body_entered(body): #this is for broken stairs
		if not open:
			if body.name != "player":
				return

		if not door.opened:
			$subtitles.text = "This stair is broken. I need to fix it"
